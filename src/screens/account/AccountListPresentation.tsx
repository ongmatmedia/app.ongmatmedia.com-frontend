...
export const AccountListPresentation = (props: AccountListPresentationProps) => {
  ...
  const isSelectingAllAccounts = props.selectedAccounts.length === props.accounts.length
    ...
    <>
      ...
      <List
        ...
        renderItem={({ id, name }) => (
          <List.Item>
            <Card
              bodyStyle={{ padding: 10, paddingBottom: 0, border: props.selectedAccounts.includes(id) ? '2px solid #1890ff' : '2px solid white', }}
              loading={name == null}
            >
              <Row type="flex" justify="start" align="middle" style={{ paddingBottom: 10 }} >
                <Col xs={12} style={{ cursor: 'pointer' }} onClick={() => props.selectedAccounts.includes(id) ? props.onSelectAccount([...props.selectedAccounts.filter(idAccount => idAccount !== id)]) : props.onSelectAccount([...props.selectedAccounts, id])}>
                  <Avatar src={`http://graph.facebook.com/${id}/picture?type=large`} size={60} />
                </Col>
                <Col xs={12} style={{ overflow: "auto", cursor: 'pointer' }} onClick={() => props.selectedAccounts.length === 0 && props.onOpenViewAccountModal(id)}>
                  <span>{name}</span>
                </Col>
              </Row>
            </Card>
