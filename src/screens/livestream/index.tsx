			...
			<Row style={{ backgroundColor: 'white', padding: 20 }}>
				<Col span={24}>
					<Tabs
						...
						onTabClick={(key: string, event: MouseEvent) =>
							setActiveTabKey(key)
						}
						defaultActiveKey="1"
						type="card"
					>
						<TabPane tab="List livestreams" key="1">
							...
							<LivestreamsListTab
								onNavigateCreateUpdateTab={() => setActiveTabKey('2')}
								onSelectLiveToUpdate={live => selectLiveToUpdate(live)}
							/>
						</TabPane>
						<TabPane tab="Create/Update livestream" key="2">
							<CreateUpdateLivestreamTab
								setActiveTabKey={setActiveTabKey}
								task={live}
								mode={live ? 'update' : 'create'}
								clearLiveCache={() => selectLiveToUpdate(null)}
							/>
						</TabPane>
