...
const ResetPasswordPage = withRouter<any, any>(
	(props: FormComponentProps & RouterProps) => {
		// Get username
		const username_match = window.location.href.match(/username=([a-zA-Z0-9]+)/)
		const [username, set_username] = useState<string | null>(
			username_match ? username_match[1] : null,
		)
		...
		useEffect(() => {
			username && send_vetification_code(username)
		}, [])
				...
				<FormItem>
					{getFieldDecorator('username', {
						...
						<Input
							...
							addonAfter={
								<Icon type="edit" onClick={() => set_username(null)} />
							}
						/>,
					)}
				</FormItem>
