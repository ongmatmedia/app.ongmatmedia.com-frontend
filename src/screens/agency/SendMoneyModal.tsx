...
export const SendMoneyModal = Form.create<SendMoneyModalProps>()(
	(props: SendMoneyModalProps) => {
		...
		const submit = async () => {
			...
			try {
				await send_money(
					...
					send_amount,
					props.user.id,
					props.user.balance + receive_amount,
				)
				set_error(null)
				set_loading(false)
				props.onClose()
		...
		const update_amount = (amount: number, is_send_amount: boolean) => {
			if (is_send_amount) {
				...
				const receive_amount = Math.floor(
					(amount * props.user.price_percent) / 100,
				)
		...
		return (
			<Modal
				...
				okButtonProps={{
					disabled:
						note == null || note.length == 0 || send_amount > props.me.balance,
					loading,
				}}
			>
				...
				>
					...
					<Col>
						<Tag color="#108ee9">
							{(props.me.balance || 0).toLocaleString(undefined, {
