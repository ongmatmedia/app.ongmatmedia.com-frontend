import { Button, Card, Col, Divider, Icon, List, Row, Skeleton, Spin, Tooltip } from 'antd'
...
const query = graphql`
	query BuffViewersLivestreamListQuery($after: String, $first: Int, $before_time: Long) {
		...BuffViewersLivestreamList_buff_viewers_livestream_tasks @arguments(after: $after, first: $first, before_time: $before_time)
	}
`

const update_playing_videos = async () => {
	const { buff_viewers_livestream_playing } = await GraphQLQueryFetcher<{ buff_viewers_livestream_playing: BuffViewersLivestreamConnection }>(
		graphql`
	...
	commitLocalUpdate(RelayEnvironment, store => {
		for (const { node } of buff_viewers_livestream_playing.edges) {
			...
			if (!logs) continue
			const new_log = store.create(`client:${node.id}:logs:${logs.length}`, 'BuffViewersLivestreamLog')
	...
	(
		...
		fragment BuffViewersLivestreamList_buff_viewers_livestream_tasks on Query
			@argumentDefinitions(after: { type: "String" }, first: { type: "Int" }, before_time: { type: "Long" }) {
				buff_viewers_livestream_tasks(first: $first, after: $after, before_time: $before_time)
				@connection(key: "BuffViewersLivestreamList_buff_viewers_livestream_tasks") {
					edges {
						node {
							id
							user_id
		...
		({ data, loading, reload, has_more, load_more, loading_more }) => {
			...
			if (loading && !data) return <Skeleton active loading paragraph={{ rows: 5 }} />
			...
			const buffStatusData = data.buff_viewers_livestream_tasks.edges.map(e => e.node.status)
			...
			const conditionStatusFilter = (condition: boolean) => !!statusFilter ? condition : true

			const timeSeriesBuffViewersLivestreamData = groupTimeIntoDayMap(
				buffViewersLivestreamTasks,
			)
				.filter(el => el.data.some(buff => !!buff.id.match(videoIdSearch) && conditionStatusFilter(buff.status == statusFilter)))
				.map(el => ({
					time: el.time,
					data: el.data.filter(buff => !!buff.id.match(videoIdSearch) && conditionStatusFilter(buff.status == statusFilter)),
				}))
										...
										renderItem={buffViewersLivestream => (
											<List.Item
												style={{
													...
													cursor: `${buffViewersLivestream.status == 'playing' ? 'pointer' : ''}`,
												}}
												onClick={() => {
					...
					{!!has_more && (
						<Row type="flex" justify="center" align="middle">
							<Col xs={24} style={{ textAlign: 'center' }}>
								<Tooltip title={videoIdSearch ? "Kết quả tìm kiếm đang bị giới hạn bởi nội dung tìm kiếm, vui lòng xóa bộ lọc để hiển thị toàn bộ kết quả." : ''} trigger="click">
									<Button
