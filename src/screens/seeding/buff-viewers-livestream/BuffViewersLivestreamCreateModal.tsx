import { Alert, Avatar, Card, Col, Icon, Input, Modal, notification, Row, Select, Spin, Tag } from 'antd'
        ...
        <Spin spinning={props.loading}>
          ...
          {props.form.field({
            ...
            render: ({ error, loading, setValues, value, set_touched, touched }) => (
              <FormElement label={t('form.facebook_video_input.title')} icon="user" error={error}>
                <VideoUrlInput
                  onSubmitVideo={info => {
                    set_video(info)
                    setValues({
          ...
          {props.form.field<number>({
            ...
            render: ({ error, loading, setValue, value, set_touched, touched }) => (
              <FormElement label={t('form.viewer_amount_select.title')} icon="eye" error={error}>
                <Select
                  onChange={setValue}
                ...
                >
                  {
                    viewers_amount.map(amount => (<Select.Option value={amount}>{amount}</Select.Option>))
                  }
                </Select>
              </FormElement>
            ),
          ...
          {props.form.field<number>({
            ...
            render: ({ error, loading, setValue, value, set_touched, touched }) => (
              <FormElement label={t('form.limit_mins_viewers.title')} icon="eye">
                <Select
                  onChange={setValue}
                ...
                >
                  {
                    limits_mins.map(amount => <Select.Option value={amount}>{amount}</Select.Option>)
                  }
                </Select>
              </FormElement>
            ),
          ...
          {props.form.field<string>({
            ...
            render: ({ error, loading, setValue, value, set_touched, touched }) => (
              <FormElement label={t('form.note_input.title')} icon="eye" error={error}>
                <Input.TextArea
            ...
            props.data && props.form.data.limit_mins && props.form.data.amount && (
              <OrderInfo
                ...
                order={[
                  ...
                  { amount: props.data.me.pricing?.buff_viewers_livestream, unit: 'credit/viewer' }
                ]}
              />
            )
          }
