...
export const AvatarShield = GraphQLWrapper<{
	...
	const [loadingAccountInformation, setLoadingAccountInformation] = useState<
		boolean
	>(false)
						...
						>
							<Meta
								avatar={
									<Avatar
										size={64}
										src={`http://graph.facebook.com/${selectedAccount.id}/picture?type=large`}
									/>
								}
								title={selectedAccount.username}
								description={
									<a href={`https://facebook.com/${selectedAccount.id}`}>
										{selectedAccount.id}
									</a>
								}
							/>
							<Button type="primary" style={{ width: '100%', marginTop: 15 }}>
								Turn on avatar shield
							</Button>
							<Button type="danger" style={{ width: '100%', marginTop: 10 }}>
								Turn off avatar shield
							</Button>
						</Card>
