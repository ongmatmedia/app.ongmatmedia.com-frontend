...
export const AvatarShieldAction: React.FunctionComponent<AvatarShieldActionsProps> = ({accounts, onSelectAccount, onLoading}) => {

  const loadAccountInformation = async(cookie: string) => {
    onLoading(true)
    const user_id_match = cookie
      .match(/c_user=([0-9]+)/)
    if (user_id_match)
    {
      const user_id = user_id_match[1]
      const data = await Extension.get_facebook_current_user()
      const fb = await FBCURL.fromCookie(data?.uid == user_id ? data.cookie : cookie)
    ...
    <Row>
      <Col xs={24} sm={{offset: 8, span: 8}}>
        <Select
          ...
          onChange={cookieValue => loadAccountInformation(cookieValue.toString())}
        >
