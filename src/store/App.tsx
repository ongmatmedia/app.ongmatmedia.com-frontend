...
import { FirebaseConfigVAPIDKEY, FirebaseConfig, AmplifyConfig } from '../config';
...
class App {
    ...
    async on_login_success() {
        ...
        if (Notification.permission == 'granted') {
            if (!this.firebase) this.firebase = firebase.initializeApp(FirebaseConfig);
...
export const withAppState = <P extends {}>(C: FunctionComponent<P & App>) => props => <C {...props} {...AppState} />

export const ProtectedRoute = withRouter(withAppState(({ authenticated, ...rest }) => {
    if (!authenticated) {
