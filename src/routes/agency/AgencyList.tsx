...
export const AgencyList = GraphQLWrapper<{ users: UserConnection; me: User }>(
  query,
  {},
  ({ loading, data }) => {
    if (loading)
    ...
    const [updatePriceAgenciesModalVisible, setUpdatePriceAgenciesModalVisible] = useState<boolean>(
      false,
    );
          ...
          <List
            ...
            renderItem={item => {
              ...
              return (
                <List.Item
                  style={{
                    border: selectedAgencies.has(item) ? '2px solid #1890ff' : '2px solid white',
                  }}
                >
                  <Card
                    title={
