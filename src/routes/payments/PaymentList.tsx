...
export const PaymentList = (props: PaymentListProps) => (
  <Fragment>
    <Table
          ...
          render: (item: PaymentHistory) => (
            <Row style={{ lineHeight: '2em' }}>
              <Col xs={24} md={8}>
                <Tag color="geekblue">
                  {' '}
                  <Moment format="D/M/Y H:m">{item.time}</Moment>
                </Tag>
              </Col>
              <Col xs={24} md={8}>
          ...
          render: (item: PaymentHistory) => (
            <Row style={{ lineHeight: '2em' }} type="flex" justify="space-around" align="middle">
              <Col xs={24} md={8}>
                <Tag color="green">
                  {' '}
                  $ = {item.balance_after.toLocaleString(undefined)}{' '}
                  <Icon
                    type="dollar"
                    style={{
                      fontSize: 16,
              ...
              <Col xs={24} md={4}>
                {item.receiver_username != 'system' ? (
                  <Tag color="#108ee9">{item.receiver_username}</Tag>
                ) : null}
              </Col>
              <Col xs={24} md={10}>
                <CopyToClipboard
      ...
      dataSource={props.payment_histories.filter(el =>
        `${el.note} ${el.service} ${el.receiver_username}`
          .toLowerCase()
