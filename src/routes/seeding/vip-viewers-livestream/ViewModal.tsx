...
import { Modal, Row, Col, Avatar, Typography, List, Skeleton, Icon, Divider, Tooltip, Spin } from 'antd'
...
const ListVideos = (props: {dataSource: VideoProps[], onClick: Function}) => (
  <List
    ...
    renderItem={({ video: { status, thumbnail, title, current_viewers, description, video_id } }) => (
      <List.Item
        actions={
          [
            status === 'loading'
              ...
              : status === 'paused'
                ? <Tooltip placement="top" title="Click to play">
                    <Icon type="pause-circle" style={{ fontSize: 25 }} onClick={() => props.onClick(video_id)} />
                </Tooltip>
                : <Tooltip placement="top" title="Click to pause">
                    <Icon type="play-circle" style={{ fontSize: 25 }} onClick={() => props.onClick(video_id)}/>
                </Tooltip>
          ]
        }
      >
    ...
    >
      <Row type="flex" align="middle">
        ...
        <Col span={20}>
          ...
          <TextCustom content={<Moment format="DD/MM/YYYY H:mm">{props.person.created_time}</Moment>} />
      ...
      <Divider />
      {
        isLoadingVideos && !videosArray ? (
          <Row align="middle" type="flex" justify="center">
            ...
            <Row>
              {videosArray && <ListVideos dataSource={videosArray} onClick={props.onClick} />}
            </Row>
          )
      }
    </Modal>
