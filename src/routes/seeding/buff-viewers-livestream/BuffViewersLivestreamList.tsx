        ...
        render: (item: BuffViewersLivestream) => (
            <Row type="flex" align="middle" justify="space-between" >
                <Col xs={24} md={4} xl={2} xxl={2}>
                    <Row type="flex" justify="space-around"><Col>
        ...
        render: (item: BuffViewersLivestream) => (
            <Row align="middle">
                <Col>
                    <Icon
                        type="video-camera"
                        style={{ color: 'black', margin: 10, fontSize: 20, cursor: 'pointer' }}
...
export const BuffViewersLivestreamList = GraphQLWrapper<{ buff_viewers_livestream_tasks: BuffViewersLivestreamConnection }>(query, {}, ({ data, loading }) => (
    <Table
        loading={loading}
        columns={columns}
        dataSource={data ? data.buff_viewers_livestream_tasks.edges.map(e => e.node) : []}
    />
))