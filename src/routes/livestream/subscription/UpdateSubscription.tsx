...
export const UpdateSubscription = withForm<UpdateSubscriptionProps>(props => {
  ...
  const old_total =
    old_subscription.concurrent_limit * old_subscription.days * old_subscription.pricing;
  ...
  const new_total =
    new_subscription.concurrent_limit * new_subscription.days * new_subscription.pricing;
          ...
          {props.form.field({
            ...
            render: ({ setValue, value }) => (
              <Slider
                ...
                min={old_subscription.concurrent_limit}
                max={100}
                step={1}
                defaultValue={value}
                tooltipVisible={true}
          ...
          {props.form.field({
            ...
            render: ({ setValue, value }) => (
              <Row type="flex" justify="start" align="middle" gutter={20}>
                {[480, 720, 1080].map(q => (
                  <Col>
                    <Button onClick={() => setValue(q)} type={value == q ? 'primary' : 'dashed'}>
                      {q}p{' '}
                    </Button>
                  </Col>
                ))}
        ...
        <Col md={24} xxl={12} sm={24} xs={24}>
          <Card>
            <Row>
              ...
              <Col span={16}>
                <Tag color="#108ee9">{old_subscription.concurrent_limit} threads</Tag>x{' '}
                <Tag color="#108ee9">{old_subscription.days} days</Tag>x{' '}
                <Tag color="#108ee9">
                  {' '}
                  {old_subscription.pricing}$ / thread / day ~ {props.sub.quality}
                </Tag>
                ={' '}
                <Tag color="geekblue">
                  {' '}
            ...
            <Row style={{ marginTop: 10 }}>
              ...
              <Col span={16}>
                <Tag color="#108ee9">{new_subscription.concurrent_limit} threads</Tag>x{' '}
                <Tag color="#108ee9">{new_subscription.days} days</Tag>x{' '}
                <Tag color="#108ee9">
                  {' '}
                  {new_subscription.pricing}$ / thread / day ~ p{props.form.data.quality}
                </Tag>
                ={' '}
                <Tag color="geekblue">
                  {' '}
