...
export const LivestreamTargetItemSelector = (props: FacebookAccountTarget) => {
    ...
    const onSelectAccount = async (acc: FacebookAccount) => {
        props.type == 'profile' ? props.onSelect(acc.id, acc.name) : (
            select_account(acc),
            load_groups_pages(acc.access_token)
        )
    }
    ...
    return (
        <span>
            ...
            <QueryRenderer
                ...
                render={rs => {
                    ...
                    const accounts: FacebookAccount[] = accounts_loading ? [] : (
                        (
                            (rs.props as any)
                                .facebook_accounts as FacebookAccountConnection
                        )
                            .edges
                            .map(e => e.node)
                    ...
                    if (accounts.length == 0) {
                        return <Alert message="Add a facebook account to system to broadcast livestream !" type="warning" />
                    }

                    const selectable_accounts = accounts.filter(a => !props.selected.includes(a.id))

                    if (selectable_accounts.length == 0) {
                        return <Alert message={`You will broadcast livestream to all your ${props.type}s !`} type="success" />
                    }
            ...
            {
                props.type != 'profile' && !loading_groups_pages && (selectable_groups_pages.length > 0 ? (
                    <Select
                        size="large"
                        showSearch
                        placeholder="Select facebook account"
