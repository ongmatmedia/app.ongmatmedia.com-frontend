...
export class FBFriend
{
	...
	{
		const {data} = await this.client.get('https://mbasic.facebook.com/pokes/friend_search', {query: friend_name})
		const regex = new RegExp(`\\/pokes\\/inline\\/\\?dom_id_replace=u_0_1&amp;is_hide=0&amp;poke_target=${friend_uid}.*hash=[^\s"-]+`, "gm")
